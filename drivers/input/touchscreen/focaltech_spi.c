			      ...
			      size_t cmd_size, void *val_buf, size_t val_size)
{
	...
	tx[0] = *cmd;
	tx[1] = FOCALTECH_SPI_READ_CMD;
	put_unaligned_be16((u16)val_size, tx + 2);
	...
	if (!memchr_inv(rx, 0, xfer.len)) {
		dev_err_ratelimited(&spi->dev, "SPI read returned all zeroes\n");
		return -ENXIO;
	}

	if (rx[FOCALTECH_SPI_STATUS_OFFSET] & FOCALTECH_SPI_ERROR_MASK)
		return -EIO;

	ret = focaltech_spi_check_crc(rx + FOCALTECH_SPI_PREFIX_LEN, val_size);
	if (ret) {
		dev_err_ratelimited(&spi->dev,
	...
	memcpy(val_buf, rx + FOCALTECH_SPI_PREFIX_LEN, val_size);
	return 0;
}
...
static int focaltech_spi_write(void *context, const void *data, size_t count)
{
	...
	if (count < FOCALTECH_SPI_COMMAND_LEN)
		return -EINVAL;
	payload_len = count - FOCALTECH_SPI_COMMAND_LEN;
	...
	tx[0] = *cmd;
	tx[1] = FOCALTECH_SPI_WRITE_CMD;
	put_unaligned_be16((u16)payload_len, tx + 2);
	if (payload_len)
		memcpy(tx + FOCALTECH_SPI_PREFIX_LEN,
...
static int focaltech_spi_probe(struct spi_device *spi)
{
	...
	if (max_size <= FOCALTECH_SPI_READ_OVERHEAD_LEN)
		return dev_err_probe(&spi->dev, -EINVAL,
				     "SPI controller transfer limit too small\n");

	regmap_config.max_raw_read = min(max_size - FOCALTECH_SPI_READ_OVERHEAD_LEN,
					 FOCALTECH_SPI_MAX_PAYLOAD_LEN);
	regmap_config.max_raw_write = min(max_size - FOCALTECH_SPI_PREFIX_LEN,
					  FOCALTECH_SPI_MAX_PAYLOAD_LEN);
