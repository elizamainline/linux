...
#define FOCALTECH_REG_CHIP_ID2			0x9f
...
#define FOCALTECH_EVENT_DOWN(f)			(((f) == FOCALTECH_TOUCH_DOWN) || \
							((f) == FOCALTECH_TOUCH_CONTACT))
#define FOCALTECH_EVENT_UP(f)			((f) == FOCALTECH_TOUCH_UP)
...
static int focaltech_get_ic_info(struct focaltech_core *cd)
{
	...
	if (cd->ic_data->has_flash) {
		for (attempt = 0; attempt < 5; attempt++) {
			ret = regmap_raw_read(cd->regmap, FOCALTECH_REG_CHIP_ID,
					      &id[0], 1);
			if (!ret)
				ret = regmap_raw_read(cd->regmap,
						      FOCALTECH_REG_CHIP_ID2,
						      &id[1], 1);
			if (!ret && id[0] == 0x56 && id[1] == 0x72) {
				dev_info(cd->dev,
					 "Detected FT3683G chip ID %02x:%02x\n",
					 id[0], id[1]);
				return 0;
			}
			fsleep(200000);
		}
		return dev_err_probe(cd->dev, ret ?: -ENODEV,
				     "FT3683G chip ID %02x:%02x\n",
				     id[0], id[1]);
	}
...
static void focaltech_report_touch_frame(struct focaltech_core *cd, u8 *buf,
					 bool v2)
{
	unsigned int flag, x, y, p, area, minor;
	u8 touch_num, id;
	size_t base;
	int limit;
	...
	limit = v2 ? touch_num : FOCALTECH_MAX_TOUCH_POINTS;
	for (int i = 0; i < limit; i++) {
		base = v2 ? 4 + FOCALTECH_ONE_TCH_LEN_V2 * i :
			    2 + FOCALTECH_ONE_TCH_LEN * i;
		...
		if (!FOCALTECH_EVENT_DOWN(flag) && !FOCALTECH_EVENT_UP(flag))
			continue;
		...
		if (FOCALTECH_EVENT_DOWN(flag)) {
			...
			p = v2 ? 0x3f : buf[base + FOCALTECH_TOUCH_OFF_PRE];
			area = buf[base + FOCALTECH_TOUCH_OFF_AREA];
			minor = v2 ? buf[base + 6] : area;

			p = p ? p : 0x3f;
			area = area ? area : 0x09;
			minor = minor ? minor : 0x09;
				     ...
				     int event_type, u8 *buf)
{
	...
	case FOCALTECH_TOUCH_DEFAULT:
		focaltech_report_touch_frame(cd, buf, false);
		break;
	case FOCALTECH_TOUCH_EVENT_NUM:
		if (cd->ic_data->touch_report_len ==
		    4 + FOCALTECH_MAX_TOUCH_POINTS * FOCALTECH_ONE_TCH_LEN_V2)
			focaltech_report_touch_frame(cd, buf, true);
		break;
...
static irqreturn_t focaltech_irq(int irq, void *data)
{
	...
	ret = regmap_raw_read(cd->regmap, FOCALTECH_TOUCH_ADDR,
			      cd->touch_buf, cd->ic_data->touch_report_len);
	if (ret) {
				      ...
				      const struct input_id *id)
{
	...
	input_set_abs_params(cd->input_dev, ABS_MT_TOUCH_MINOR, 0, 255, 0, 0);
...
static int focaltech_power_on(struct focaltech_core *cd)
{
	...
	/* Keep reset asserted while both rails settle. */
	gpiod_set_value_cansleep(cd->reset_gpio, 1);
	fsleep(2000);
	ret = regulator_bulk_enable(ARRAY_SIZE(focaltech_supplies), cd->supplies);
	if (ret)
		return ret;
...
static int focaltech_resume(struct device *dev)
{
	...
	ret = cd->ic_data->has_flash ? focaltech_get_ic_info(cd) :
		focaltech_load_firmware(cd);
	if (ret) {
		focaltech_power_off(cd);
		return ret;
	}
		    ...
		    const struct focaltech_ic_data *ic_data)
{
	...
	if (!ic_data || !ic_data->touch_report_len)
		return -EINVAL;

	cd->touch_buf = devm_kzalloc(dev, ic_data->touch_report_len, GFP_KERNEL);
	if (!cd->touch_buf)
		return -ENOMEM;
	...
	cd->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
	...
	/* Flash-backed FT3683G was reset when its supplies were enabled. */
	if (!ic_data->has_flash && !ic_data->is_in_cell)
